...
- [~] Phase 5.3: **Open-source toolchain — Yosys + nextpnr-generic + FASM (PARTIALLY OPEN, arithmetic designs now hardware-verified via Phase 5.4)**. Target: replace Quartus with `Verilog → Yosys → nextpnr-generic → np2fasm → fasm2rbf → openFPGALoader`. Current state:
  ...
  - **IOB_ROUTE FASM directive + frame-split bridge (2026-04-14)**: `IOB_ROUTE PIN_X -> XaYbNc.port` wired into `fasm2rbf` (6/6 tests); CRAM band bit-perfect vs the HW-verified pair RBF. `IOB_BASELINE_NV` (132-bit-cell / 74-byte hdr bridge from `nv_zero_global` to `iob_in_E15`) and `IOB_CLK_INPUT PIN_E1` (40-cell clock-bank pin activate) close the frame-split so end-to-end FASM designs can build on a single `nv_zero_global` base. `scripts/iob_slice_mining/build_simple_led_fasm.py` demonstrates the full stack (KEY=E16 → LUT@X10Y4N0 → DFF → LED=G15) with hdr = 0 diffs vs Quartus gold; remaining CRAM diffs are pair-template secondary-LE residue (characterized, not a codec bug). Tests: `fuzz/test_iob_baseline_nv_directive.py` 9/9.
