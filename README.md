...
- [~] Phase 3.19: Map remaining R4 I-indices — **24 of 37 mapped** (I=6 removed 2026-04-08 after Option-1 fingerprint recheck proved it was a propagation error from I=8 with zero independent evidence; I=8 validated at 83.3%). **Mass audit 2026-04-08** (Methods B+D) flagged 11/16 testable entries SUSPECT at <40% hit rate — the R4 formula is **dead code for synthesis** because `route_synth.emit_ops()` short-circuits via the signature backend for the entire green corpus; regression stays 686/686. A proper re-mine needs `r4_remine.py` (CRC-normalized, ≥3 prev_x, cross-seed). 13 remain unmapped (5,6,9,24,28,29,30,31,32,33,104,116,125)
