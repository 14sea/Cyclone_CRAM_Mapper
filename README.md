...
- [~] Phase 5.3: **Open-source toolchain — Yosys + nextpnr-generic + FASM (PARTIALLY OPEN, arithmetic designs now hardware-verified via Phase 5.4)**. Target: replace Quartus with `Verilog → Yosys → nextpnr-generic → np2fasm → fasm2rbf → openFPGALoader`. Current state:
  ...
  - **GCLK pipeline landed + HW verified (2026-04-14)**: `GCLK_PIN` + `LAB_CLK_SEL` + `LAB_CLK_SEL_LE` FASM directives compose as XOR-delta on an AUTO-mode baseline; source encoding is per-pin one-hot (E1=3 cells, R8=5, N1=38; zero cross-pin overlap). 26 (LAB, N) combinations round-trip bit-perfect; `fasm2rbf` 10/10 + `np2fasm` 6/6 tests. Retiring the legacy `nv_zero_global.rbf` baseline is pending the next HW flash pass.
  ...
  - **IOB_ROUTE FASM directive + frame-split bridge + single_le override (2026-04-15)**: `IOB_ROUTE PIN_X -> XaYbNc.port` wired into `fasm2rbf` (7/7 tests); CRAM band bit-perfect vs the HW-verified pair RBF via `absolute_cells`. `IOB_BASELINE_NV` (132-bit-cell / 74-byte hdr bridge from `nv_zero_global` to `iob_in_E15`) and `IOB_CLK_INPUT PIN_E1` (40-cell clock-bank pin activate) close the frame-split so end-to-end FASM designs can build on a single `nv_zero_global` base. An opt-in `single_le_cells` section in `results/iob_to_slice_sigcache.json` overrides `absolute_cells` when present and strips pair-template secondary-LE decoration for single-LE designs — derived by solving `IOB_ROUTE_primary = gold_delta ^ (all other directives)` against the Quartus gold RBF. `scripts/iob_slice_mining/build_simple_led_fasm.py` end-to-end simple_led (KEY=E16 → LUT@X10Y4N0 → DFF → LED=G15) now produces an RBF byte-identical to Quartus gold (hdr = 0, CRAM = 0, trl = 0) via the full 8-directive stack. Tests: `fuzz/test_iob_baseline_nv_directive.py` 9/9 + `fuzz/test_iob_route_directive.py` 7/7.
