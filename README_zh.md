...
- [~] Phase 5.3：**开源工具链 —— Yosys + nextpnr-generic + FASM（部分开通）**。目标：用 `Verilog → Yosys → nextpnr-generic → np2fasm → fasm2rbf → openFPGALoader` 取代 Quartus。当前状态：
  ...
  - **IOB_ROUTE FASM 指令 + 帧分裂桥接（2026-04-14）**：`IOB_ROUTE PIN_X -> XaYbNc.port` 已接入 `fasm2rbf`（6/6 测试），CRAM 带逐 byte 与硬件验证过的 pair RBF 对齐。`IOB_BASELINE_NV`（`nv_zero_global` → `iob_in_E15` 的 132-bit-cell / 74 byte header 桥接 delta）与 `IOB_CLK_INPUT PIN_E1`（40 cell 的专用时钟 bank pin 激活 delta）一起把帧分裂补齐，让端到端 FASM 设计可以直接在 `nv_zero_global` 单一基底上构建。`scripts/iob_slice_mining/build_simple_led_fasm.py` 演示了完整指令栈（KEY=E16 → LUT@X10Y4N0 → DFF → LED=G15），header 带 0 diff 对齐 Quartus gold；剩余 CRAM diff 是配对模板的副 LE 装饰残留（已定性，非 codec bug）。测试：`fuzz/test_iob_baseline_nv_directive.py` 9/9
