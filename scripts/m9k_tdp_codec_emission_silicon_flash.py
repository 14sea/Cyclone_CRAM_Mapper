...
def main():
    site = "X15_Y10_N0"
    # Quartus gold for TDP 16x32 at X15_Y10_N0 (from m9k_tdp_blink_build.py
    # default site).  Already HW-validated as Quartus flash; here we test
    # the codec-emission path that re-uses gold's bit cells via XOR onto
    # nv_zero_global.
    gold_path = ROOT / "tmp/m9k_tdp_blink_16x32/m9k_tdp_blink_16x32.rbf"
    nv_path = ROOT / "results/rbf/nv_zero_global.rbf"
    out_path = ROOT / f"tmp/codec_emission_tdp_blink_rebuilt_{site}.rbf"
    ...
    print(f"\nResidual vs TDP gold: {len(residual)} cells")
    ...
    if len(final_violations) == 0:
        ...
        print("  Gold is HW-validated (m9k_mode_tdp_hw_validated_2026_04_25); "
