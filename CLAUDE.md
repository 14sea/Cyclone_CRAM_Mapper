...
| `M9K_MODE_{w}x{d}[_template]` | `X15Y10N0.M9K_MODE_9x512_inferred_goldintersect` | **HW-VALIDATED at (9,512) + (18,512) + (9,1024) + (36,256) + (4,2048-CLEAN23).** np2fasm `_emit_m9k_mode` gate: `_M9K_MODE_HW_VALIDATED = {(9,512),(18,512),(9,1024),(36,256),(4,2048)}`; other widths warn+skip. Parser/loader/bitgen + `_emit_m9k_mode` helper (`fuzz/fasm2rbf.py`, `synth/np2fasm.py`). Template suffixes: `_altsyncram`, `_inferred`, `_inferred_goldintersect` (site-invariant `inferred` ∩ Quartus smoke gold — 38 cells at w=9, 74 at w=18, 79 at (9,1024), 11 at (36,256), 24 at (4,2048) raw / 23 after silicon mask). HW validation pattern: XOR-overlay on cff800e HW-PASS w=9 RBF at `scripts/stage0_flash_bundle/simple_led_m9k_mode_goldintersect.rbf`; per-width overlay builders at `build_simple_led_m9k_mode_{w}x{d}_gi_probe_overlay.py`. **(4,2048) silicon mask**: raw 24-cell gi bucket FAILed on 2026-04-24 (LED0 stuck on, KEY2 inert); bisection at `scripts/stage0_flash_bundle/build_m9k_mode_w4x2048_bisect.py` (6 layers on AX301) isolated an adjacent-byte pair interaction at frame 1733 — (364092,2)+(364093,2) both flipped leaks, either singleton passes; loader drops (364093,2) via `_M9K_MODE_SILICON_FALSIFIED[(4,2048)]` and CLEAN23 PASSes (byte-identical verifier at `scripts/stage0_flash_bundle/verify_m9k_mode_w4x2048_loader_mask.py`). Tests: `fuzz/test_m9k_init_directive.py` + `fuzz/test_np2fasm_m9k.py` (15+13). Memory: `m9k_mode_w4x2048_bisect_clean23.md`, `m9k_mode_widths_hw_validation_2026_04_24d.md`, `m9k_mode_template_residual.md`, `stage0_round2_flash_results.md`. |
