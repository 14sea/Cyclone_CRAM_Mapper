...
| `LUT_ARITH_MULTI_LAB` | `LUT_ARITH_MULTI_LAB WIDTH=17` | OK — widths 17..32 multi-LAB chain activation **SILICON-VALIDATED at W=17** (LED0 solid-on at LAB(4,18)+(4,17), 2026-05-03 night).  Codec asset: `results/arith_blockband_by_width.json` `multi_lab["16+1..16+16"]`.  Includes carry-input LI MUX cells at LAB(4,17) (pairs 0/1/4/5).  **Consumer dedup required**: LUT_ARITH_MULTI_LAB shares cells with OUTROUTE_G15 sigcache (chain MSB output route shares routing infrastructure with carry-input MUX) — fasm2rbf's `_iob_route_dedup` set defeats sequential XOR cancellation.  Validator extended with `paired_carry_input` mode for the all-paired no-P8 LI pattern Quartus emits at multi-LAB carry-receiver LABs.  np2fasm consumer NOT wired (Yosys `$alu` techmap caps at single-LAB; CE6_CARRY techmap extension + nextpnr cout→cin pip routing for multi-LAB needed).  See memory `multi_lab_carry_silicon_validated_2026_05_03.md`. |
