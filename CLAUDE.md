...
|-------|-------------|-------|
| `LutCodec` | `write_tt(base_rbf, mask)` → RBF, `read_tt(rbf, zero)`, `from_cram_model(x,y,n)` | **XOR-delta, not absolute!** `mask = target ^ base_tt`. `from_cram_model` uses σ⁻¹ permutation lookup keyed by `(foff, fb8, group)` — 2112 entries, 5-level fallback: exact 3-key → nearest-foff 3-key → exact 2-key → nearest 2-key → identity. Data: `results/sigma_inv_fb8_groups.json`. Residual: fb8=7 × group=4 is silicon-geometry blocked (X=8 has no LAB at Y≥12). σ⁻¹'s 16 claimed TT cells per LE are real TT (mining-verified 2026-05-05, `results/real_tt_classification.json`).  **Re-audit 2026-05-07 (memo `sigma_inv_extension_unneeded_2026_05_07`)**: the earlier "σ⁻¹ also misses 10-20 additional real TT cells" claim was MISFRAMED — those extra cells are header-band (frame 0, off ∈ {42-52, 73, 74} only) per-design IOB+CLK+route metadata, NOT TT bits.  Owner-trace via `scripts/sigma_inv_real_tt_mining/trace_missed_cell_ownership.py` confirms all 12 W=23 frame-0 diff cells are XOR-parity=1 owned by existing directives (IOB_PAD_NV / IOB_PAD_ARITH_EXT / IOB_CLK_INPUT / OUTROUTE_G15).  σ⁻¹ codec is complete for the LUT-TT scope.  See memory `sigma_inv_3key_discovery.md` + `phase3_fix_silicon_validated_2026_05_05.md`. |
...
13. **Phase 3 LI MUX std_lut TT collision — FIXED 2026-05-05 (silicon-validated).** Phase 3 unconditionally restored 18 LI MUX cells per LAB at a `(group, slot)`-derived bp via `_cram_group_bit(y)`. For Y where Phase 3 bp == LE's LUT TT bp at the same byte offsets (Y ∈ {2,4,5,7,10,14,17} hit; Y ∈ {3,6,18,21} safe), the restoration overwrote Phase 1+2's LUT TT writes with the post-apply_routing snapshot — `X4Y4N0.LUT = 0xaaaa` decoded as 0xF0F0 ("pass datac" → output stuck → LED constantly on, silicon-validated 2026-05-04). **Mining via `scripts/sigma_inv_real_tt_mining/mine_real_tt.py`** (data: `results/real_tt_classification.json`) across 8 (X,Y) positions — all at LE_0 (n=0) — covering colliding Y ∈ {4,10,17} and safe Y ∈ {3,6,14} proved **σ⁻¹'s 16 claimed TT cells are 100% real TT at LE_0** (correctly_identified=16, misclassified=0 per LE). N>0 LE positions are not directly verified by this mining; the fasm2rbf Phase 3 fix is N-symmetric so the result is expected to generalize, but a follow-up mining batch over n∈{2,4,...,30} would close the audit (note: original mine_real_tt.py had a `quartus_n = n // 2` conv bug — Pitfall #15-class — fixed 2026-05-10 commit; n=0 silicon-tolerated 0//2=0=correct LE_0) — they DO flip when LUT mask changes between XOR4 (0x6996) and XNOR4 (0x9669). The original 2026-05-02 "Phase 3 fixes σ⁻¹ misclassification" rationale was wrong: σ⁻¹ doesn't misclassify. Real bug: the 18-cell-per-LAB Phase 3 formula overestimates LI MUX — only 2 of 18 are actually LI MUX, the other 16 are real TT cells of the LE. (**Earlier "σ⁻¹ misses 10-20 additional real TT cells" claim was MISFRAMED — see LutCodec row above + memo `sigma_inv_extension_unneeded_2026_05_07`.**) **Fix**: `fuzz/fasm2rbf.py` Phase 3 builds `std_lut_tt_cells` (union of `tt_cells_cache[(x,y,n)]` for all non-arith std_luts) and skips those `(off, bp)` keys in the `li_locked_state` restoration loop. LUT_ARITH LEs unaffected (Phase 1 skips arith). **Silicon-validated** via hybrid RBF (Quartus gold + 12 σ⁻¹ TT bit flips from post-fix codec) flashed AX301 2026-05-05 — DONE asserted (no self-protection reset → post-fix TT bits electrically safe), LED behavior matches predicted hybrid function (LE_A 0x8888 with gold's LI MUX K2→dataa, K3→datad → LED follows K2 only, K3 ignored). Earlier cross_lab_open self-protection reset was from cross_lab_open's 3 pre-existing infrastructure gaps (memo `cross_lab_cascade_silicon_failed_2026_05_04.md`), NOT from this fix. Regression: `fuzz/test_phase3_li_mux_lut_tt.py` (11 CE6 LAB_Y values incl. all 7 colliding + 4 safe round-trip 0xaaaa). All silicon-validated builds preserved post-fix (md5 905dfc85 / d4073d2e / 47ad9ed9; ζ regression 7/7 green). Memory: `phase3_li_mux_lut_tt_collision_2026_05_04.md`, `phase3_fix_silicon_validated_2026_05_05.md`.
...
16. **Cross-LAB R4 sigcache cells × IOB_PAD_NV / LAB_CLK_SEL_LE XOR-cancel** — Bug #1 PARTIALLY FIXED 2026-05-11 (silicon-blocking, discovered via end-to-end test 2026-05-10).  The Plan D' factory mining template (`mine_x4_cross_lab_route.py` + `gen_two_luts_single_input_clocked` + FUZZ_PINS) bakes IOB and clock infrastructure cells into the cross-LAB ROUTE diff.  In open-toolchain builds those cells are also emitted by `IOB_PAD_NV` and `LAB_CLK_SEL_LE` directives → **double XOR-flip cancels both** → cells absent from final RBF → silicon would fail.  Measured for entry `4,4,0->4,21,0,dataa` (141 cells): 56 cancelled by IOB_PAD_NV + 7 by LAB_CLK_SEL_LE X4Y21 = 63 missing in open-toolchain RBF vs Quartus reference.  Affects **96/109 X=4 cross-LAB sigcache entries** (all "big" entries with cells>100 / ~40-44% IOB_PAD_NV overlap); only 13 "small" datab N=0→N=0 entries (C4-direct wires, no IOB infra in diff) are clean.  Same problem expected to apply to other Plan D' columns (X=10/16/22/25/28/31).  **Compounding Bug #2** at the same site: `iob_to_slice_sigcache.json` lacks `IOB_E16->4,4,0,dataa` and `IOB_M16->4,4,0,datab` → np2fasm emits ZERO `IOB_ROUTE` directives — but Bug #1 mining-baked routing partially compensates (E16→lut1 cells live inside the ROUTE sigcache); silicon impact of Bug #2 standalone is unverified.  Test fixture: `scripts/sigcache_validation/build_test.py` (re-runnable; pinned LE positions force the bug to surface).  **Fix landed (Bug #1)**: `scripts/sigcache_remine/strip_iob_overlap.py --apply --column 4` post-processes the 96 affected X=4 entries by subtracting `IOB_PAD_NV ∪ LAB_CLK_SEL_LE(src,sn) ∪ LAB_CLK_SEL_LE(dst,dn) ∪ LAB_CLK_SEL(src) ∪ LAB_CLK_SEL(dst)` from each entry; runtime IOB_PAD_NV/LAB_CLK_SEL_LE directives then own those cells without double-flip.  Both `nv_route_cells.json` and `route_cells_full.json` are patched in lockstep with automatic backups.  Re-running build_test post-fix: **63 missing → 8 fabric-band missing + 13 header-band missing** (vs Quartus reference; ζ regression 7/7 green, test_phase3 11/11 PASS, green_zone_harden (4,4) island 72/72 bit-perfect, test_fasm_roundtrip 2035/2035 OK).  Residual breakdown (post-fix gold-only):  **4 fabric cells ROUTE×OUTROUTE_G15 XOR-cancel** (37454,0)/(38505,0)/(38714,0)/(362711,3) at src-LE column LI-MUX byte positions — happens because OUTROUTE_G15 X4Y21N0 (49 cells) overlaps the ROUTE post-strip set at 14 cells, 4 of which are gold-flipped (parity=0 in open).  **2 fabric IOB_PAD_NV cells (83503,4)/(83573,4) clobbered by Phase 3 LI MUX restore** — IOB_PAD_NV-flipped cells happen to map to LI MUX positions for X=10 LAB Y=10/Y=11 P8 base (not the dst LAB), and `li_locked_state` resets them post-IOB_PAD_NV.  Known pre-existing fasm2rbf behavior; X33Y4_CROSS_LAB_SHIM works around it for X33 designs (memo `x33_silicon_validated_2026_05_03`).  **2 fabric cells (38504,0)/(38715,0) in LAB_CLK_SEL_LE X4Y21N0 not surviving** — root cause un-investigated; cells have parity=1 across all directive sets I instrumented but don't appear in open RBF.  **13 header-band cells (frame 0)** — codec gaps unrelated to Bug #1 (no directive emits these cells; pre-existing). To extend fix: (1) strip OUTROUTE_G15 cells from ROUTE sigcache when dst LE drives G15 (context-dependent — only safe to apply per-build via fasm2rbf, not at sigcache level); (2) extend Phase 3 LI MUX skip list to cover IOB_PAD_NV cells; (3) deeper investigation of (38504/38715/38505/38714) restoration path.  Audit-projected post-fix: 63 missing → 4 residuals (the memo's prediction); actual achieved: 8 fabric.  No flash performed (fabric residuals still silicon-critical).  Memo: `gamma_silicon_validation_codec_bug_2026_05_10` + `gamma_bug1_strip_fix_landed_2026_05_11`.  Pitfall #14 (context-dependent mined cells) reframed: applies in EVERY design where both directives emit, not just dense designs.  **Strip script integration**: rerun `scripts/sigcache_remine/strip_iob_overlap.py --apply --column X` after any future `nv_sig_cache_merge.py` invocation when X column has Plan D' mined entries (script is idempotent).
