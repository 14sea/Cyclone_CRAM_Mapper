...
| `M9K.INIT_{w}x{d}` | `X15Y10N0.INIT_9x512 = 0x...` | OK at 33 calibrated anchors (X∈{15,27}, Y∈[4..23]). Codec: `fuzz/m9k_init_basis.write_init`. np2fasm emits via `_emit_m9k_init`. Byte-identical vs Quartus at X15_Y10_N0 18x512 via `scripts/m9k_e2e_smoke.py`. |
| `M9K_MODE_{w}x{d}[_template]` | `X15Y10N0.M9K_MODE_9x512_quartus_gold` | **HW-validated** (SP). `quartus_gold` buckets mined via `scripts/m9k_mode_quartus_gold_mine.py` (single) or `scripts/m9k_mode_quartus_gold_batch.py --neorv32` (sweep). Per-(w,d,site) = intersection of 3 Quartus altsyncram variants vs matched no-M9K baseline, block-band only. **np2fasm gate** (`_M9K_MODE_FUNCTIONAL_VALIDATED_{SP,SDP,TDP}` × lazy-loaded site set from `results/m9k_mode_bits.json`) covers all 27 NEORV32 M9K sites + X15 Y10..Y14 SP (8,64) cache. SDP/TDP modes via `--mode sdp|tdp`; `np2fasm._emit_m9k_mode` dispatches on techmap MODE param. Site-specificity is real — reusing a bucket across sites leaks ~20+ wrong cells. HW flash of SDP/TDP still pending. Full history: memory `m9k_mode_*` entries. |
