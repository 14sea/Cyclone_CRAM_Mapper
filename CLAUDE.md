...
16. **Cross-LAB R4 sigcache × IOB_PAD_NV / LAB_CLK_SEL_LE XOR-cancel** — Bug #1 PARTIALLY FIXED 2026-05-11 (`df335d8`). Plan D' factory mining bakes IOB+CLK infra cells into the cross-LAB ROUTE diff; runtime `IOB_PAD_NV` + `LAB_CLK_SEL_LE` emit the same cells → double XOR-flip → cells silently absent → silicon failure. Affects 96/109 X=4 entries (>100 cells / ≥30-cell overlap); same class expected at X=10/16/22/25/28/31. Fix: `scripts/sigcache_remine/strip_iob_overlap.py --apply --column X` subtracts `IOB_PAD_NV ∪ LAB_CLK_SEL_LE(s/d) ∪ LAB_CLK_SEL(s/d)` per-entry; idempotent, patches `nv_route_cells.json` + `route_cells_full.json` in lockstep. **Re-run after every `fuzz/nv_sig_cache_merge.py`.** Post-fix `build_test` (entry `4,4,0→4,21,0,dataa`): 63 missing → 8 fabric + 13 header residuals.  **Codec follow-ups landed 2026-05-11**: `03b91c6` closes 4 fabric cells (ROUTE×OUTROUTE_G15 strip-at-emit + Phase 3 LI MUX skip extended to IOB_PAD_NV / OUTROUTE_G15, with X33Y4 shim exemption); `93f9cb9` gates `off<5282` hdr-skip on `needs_dedup` so padnv-bucket IOB_ROUTE entries can carry header cells (preparation for Bug #2 X=4 closure once σ⁻¹ is fixed).  ζ 7/7, test_phase3 11/11, green_zone 731/731, roundtrip 2035/2035; silicon-validated md5s preserved (W=23 `905dfc85`, probe2 `d4073d2e`, cross_lab_x33 `47ad9ed9`). **Silicon evidence (2026-05-11 PM)**: Y=7 build_test flash triggered **config-controller self-protection reset** (1/3 flash budget used), not stuck-LED — strongly suggests silicon-hostile content in OUTROUTE_G15 X4Y7N0 (freshly mined this session, never silicon-validated) or compounded by σ⁻¹ permutation at X4Y4N0 LE_0 (LE_A position).  **σ⁻¹ permutation scope (expanded 2026-05-11)**: broken at X4Y4N0 LE_0 for **both 0x8888 (read=0xC0C0) and 0xAAAA (read=0xF0F0)** — affects asymmetric masks in general, including build_test's own LE_A mask 0x8888.  Pitfall #13 closure only verified symmetric masks (XOR4 0x6996).  Implication: build_test silicon-blocked at the SRC LE regardless of dst Y choice; need σ⁻¹ permutation re-mining (research-class) to unblock.  **Do not flash X=4 cross-LAB / src-LE builds until σ⁻¹ permutation closed.** Test fixture: `scripts/sigcache_validation/build_test.py`. Memos: `gamma_silicon_validation_codec_bug_2026_05_10`, `gamma_bug1_strip_fix_landed_2026_05_11`, `gamma_y7_silicon_reset_2026_05_11`, `iob_route_hdr_skip_pivot_2026_05_11`. Pitfall #14 reframed: context dependency applies in every design where both directives emit.
