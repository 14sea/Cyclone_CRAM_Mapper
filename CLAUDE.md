...
16. **γ X=4 cross-LAB — 1-input bypass + 2-input canon BOTH CLOSED 2026-05-13 at X4Y4N0.**
    ...
    - **σ⁻¹ 2-input cross-LAB (P5b/P5c/P5d) — SAFETY UNBLOCKED 2026-05-21; FLASH HELD pending review.** P5b mined `canon_unique[L]` sidecar via `--canon-unique` flag + `CANON_2INPUT_UNIQUE` table + `canon_2input_unique_aware=1` pragma; P5c shrank per-label cells 175→49–65 by switching to wire4 baseline (`canon_const0_X4Y4N0_wire4.rbf` from `probe_canonicalization_cells.py --constants --wire4`). P5c apply-time overlap filter in `bitgen` skips canon_unique cells already touched by codec-elsewhere directives (drops cross-LAB analytical mismatch 41→12), but at P5c stop `validate_safe_for_hardware` rejected: LAB (4,4) hit 10 LI MUX cells > 9 alternating cap (4 canon_unique LI cells + 6 codec ROUTE LI cells; the 4 alone classify as safe `edge_pair_groups_b0`, the combination flips the classifier into invalid `alternating`). **P5d (2026-05-21)** extended the filter with a per-LAB LI envelope check: new `bitstream._LI_CELL_TO_LAB` reverse map + `li_lab_for_offset()` let `fasm2rbf` build `codec_li_labs` from `codec_touched` and drop canon_unique LI-class cells at any LAB where codec already emitted LI activity. Cross-LAB build now passes safety (0 violations) AND analytical gate (**10/61** real canon-cell mismatches ≤ 30 = CANDIDATE; raw 16/61 included 6 per-build variable header bytes [0x29..0x35) + [0x49..0x4B) that D4's `rbf_safe_byte_ranges.md` mandates excluding — see 2026-05-21 audit footnote in p5d memo). LAB (4,4) post-P5d: 6 LI cells, mode `paired_carry_input`. Single-LE byte-identity preserved (24/24 + 5/5) because in single-LE context codec emits 0 LI cells → P5d filter doesn't fire. Cross-LAB RBF md5 `ae9aaef6cba4d48ab4a10f78ac994aef`. Parallel blocker still open: `IOB_E16/M16 -> X4Y4N0.dataa/datab` sig-cache missing. **No flash burned; 1/3 remaining.** Memo: `p5d_per_lab_li_filter_2026_05_21`.
