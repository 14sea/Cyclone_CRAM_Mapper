...
| `M9K_MODE_{w}x{d}[_template]` | `X15Y10N0.M9K_MODE_9x512_m9k_blink_diff_nv` | ⚠️ Most codec-emission paths silicon-broken; new `m9k_blink_diff_nv` template in landing. (1) `_quartus_gold*` + `--base nv` (production np2fasm): bucket mined relative to `matched_baseline` but applied as XOR onto `nv_zero_global` → ~85% bit inversion. Every codec-emission flash silicon-resets. See memory `m9k_mode_codec_silicon_broken_2026_04_25` + `m9k_mode_d2_falsified_2026_04_26`. (2) `_inferred_goldintersect` + `--base pure`: previously claimed HW-validated based on overlay tests at w=9/w=18 (2026-04-17, 2026-04-24), but those proved fabric-safety only — NOT mode-correctness. ζ-swap silicon test 2026-04-30 (memory `m9k_mode_v5_finding_gi_codec_unnecessary_2026_04_30`) proved the gi codec targets cells the M9K mode hardware doesn't read; v5 (drop gi entirely, keep only the 12 non-inferred Quartus block-band cells per site) silicon-PASSED. (3) **`_m9k_blink_diff_nv` (current path)**: per-site bucket mined as `(m9k_blink_full_at_site ⊕ nv_zero_global)` restricted to block-band frames 1692-1738 (byte<208). ~12 cells per site (slot=1 sites such as X15_Y6 give ~6). Mining via `scripts/m9k_blink_diff_nv_mine.py` (after `scripts/m9k_blink_full_build.py` produces the per-site Quartus reference). 9 X=15 SP 9×512 sites mined (2026-04-30 PM); FASM-driven byte-identity test (`scripts/m9k_diff_nv_fasm_test.py`) PASS at all 9; X15_Y4/Y11/Y6 silicon-flashed. SP 9×512 mode-correctness already proven by v5 ζ-swap at X15_Y10. See memory `m9k_blink_diff_nv_landed_2026_04_30.md`. **Pending**: extend to TDP/SDP/wider modes (NEORV32 needs ROM 32×256, SP 8×64, SDP 8×1024×16, TDP 16×32) by adding mode variants of `m9k_blink_full_build.py`; build remaining sites (X15 Y={2,3,13..23} + entire X27 column); then np2fasm pivot (Task #5) to swap `_MODE_TO_BUCKET` to `m9k_blink_diff_nv`. Quartus-gold direct flash (NOT codec-emission) still works for ζ pipeline use cases. SDP/TDP `quartus_gold_{sdp,tdp}` buckets mined via `scripts/m9k_mode_quartus_gold_mine.py --mode {sdp,tdp}`. Full history: memory `m9k_mode_*` entries. |
