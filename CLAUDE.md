...
| `IOB_ROUTE` | `IOB_ROUTE PIN_E16 -> X10Y4N0.dataa` | OK — pin→LE-port sig lookup. `absolute_cells` reproduces HW-verified pair RBF byte-perfect; optional `single_le_cells` override strips pair-template secondary-LE decoration for single-LE designs (E16→10,4,0,dataa: derived 164 cells → simple_led full-RBF 0 diffs vs Quartus gold). |
