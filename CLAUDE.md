...
16. **Cross-LAB R4 sigcache cells × IOB_PAD_NV / LAB_CLK_SEL_LE XOR-cancel** (silicon-blocking, discovered via end-to-end test 2026-05-10).  The Plan D' factory mining template (`mine_x4_cross_lab_route.py` + `gen_two_luts_single_input_clocked` + FUZZ_PINS) bakes IOB and clock infrastructure cells into the cross-LAB ROUTE diff.  In open-toolchain builds those cells are also emitted by `IOB_PAD_NV` and `LAB_CLK_SEL_LE` directives → **double XOR-flip cancels both** → cells absent from final RBF → silicon would fail.  Measured for entry `4,4,0->4,21,0,dataa` (141 cells): 54 cancelled by IOB_PAD_NV + 5 by LAB_CLK_SEL_LE X4Y21 + 4 unaccounted = 63 missing in open-toolchain RBF vs Quartus reference.  Affects **96/109 X=4 cross-LAB sigcache entries** (all "big" entries with cells>100 / ~40-44% IOB_PAD_NV overlap); only 13 "small" datab N=0→N=0 entries (C4-direct wires, no IOB infra in diff) are clean.  Same problem expected to apply to other Plan D' columns (X=10/16/22/25/28/31).  **Compounding Bug #2** at the same site: `iob_to_slice_sigcache.json` lacks `IOB_E16->4,4,0,dataa` and `IOB_M16->4,4,0,datab` → np2fasm emits ZERO `IOB_ROUTE` directives → KEYs disconnected from LE_A → silicon fails regardless of #1 fix.  Test fixture: `scripts/sigcache_validation/build_test.py` (re-runnable; pinned LE positions force the bug to surface).  **Fix plan**: post-process sigcache to subtract IOB_PAD_NV ∪ LAB_CLK_SEL_LE cells; mine missing IOB_ROUTE entries via `sweep_single_le.py`; add fasm2rbf defensive dedup as future hardening.  Audit-projected post-fix: 63 missing → 4 residuals.  Memo: `gamma_silicon_validation_codec_bug_2026_05_10`.  Pitfall #14 (context-dependent mined cells) reframed: applies in EVERY design where both directives emit, not just dense designs.
