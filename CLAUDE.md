...
| `LUT_ARITH_MULTI_LAB` | `LUT_ARITH_MULTI_LAB WIDTH=17` | OK (codec) — widths 17..32 multi-LAB chain activation.  Mining-time QSF bug fixed 2026-05-03 night (`Q[i]~reg0` → `Q[i]`); `results/arith_blockband_by_width.json` `multi_lab["16+1..16+16"]` regenerated, all c_ml builds W15706-clean, atom traversal confirms chain at FF_X4_Y18_N* + FF_X4_Y17_N1.  Static-diff of `visible_blink_W17.rbf` vs nv_zero_global confirms cells now land in col_4/col_3 (LAB(4,18)+(4,17)).  **Hand-FASM caveat**: chain-input LI MUX cells at LAB(4,17) (pairs 0,1) are NOT in this codec asset — they're routing cells separate from block-band arith activation. visible_blink hand-FASM at LAB(4,18)+(4,17) is currently `validate_safe_for_hardware`-UNSAFE due to missing P0 anchor; Quartus gold also violates validator (different pattern), indicating the validator's known-safe envelope is incomplete for multi-LAB carry chains.  np2fasm consumer NOT wired (Yosys `$alu` techmap caps at single-LAB).  See memory `multi_lab_codec_fixed_2026_05_03.md` (current) + `multi_lab_codec_broken_2026_05_03.md` (historical RCA) + `arith_width_17_32_landed.md` (legacy). |
