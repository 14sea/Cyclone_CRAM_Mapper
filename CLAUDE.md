...
13. **Phase 3 LI MUX restoration in fasm2rbf collided with std_lut LUT TT — FIXED 2026-05-05.** Originally (until commit landing this fix) Phase 3 unconditionally restored 18 LI MUX cells per LAB at a `(group, slot)`-derived bp via `_cram_group_bit(y)`; for Y values where that bp equalled the LE LUT TT bp at the same byte offsets (Y ∈ {2,4,5,7,10,14,17} hit; Y ∈ {3,6,18,21} safe), the restoration overwrote Phase 1+2's LUT TT clear+XOR with the post-apply_routing snapshot — `X4Y4N0.LUT = 0xaaaa` decoded as `0xF0F0` ("pass datac" with datac unconnected → output stuck 1 → LED constantly on, silicon-validated 2026-05-04). Fix: `fuzz/fasm2rbf.py` Phase 3 builds `std_lut_tt_cells` (union of `tt_cells_cache[(x,y,n)]` for all non-arith std_luts) and skips those `(off, bp)` keys in the `li_locked_state` restoration loop. LUT_ARITH LEs unaffected (Phase 1 skips arith; arith snapshot IS the desired runtime state). Regression: `fuzz/test_phase3_li_mux_lut_tt.py` (11 CE6 LAB_Y values incl. all 7 colliding + 4 safe round-trip 0xaaaa). All silicon-validated builds preserved post-fix (md5 905dfc85 / d4073d2e / 47ad9ed9; ζ regression 7/7 green). Memory: `phase3_li_mux_lut_tt_collision_2026_05_04.md`, `phase3_li_mux_lut_tt_fix_landed_2026_05_05.md`.
