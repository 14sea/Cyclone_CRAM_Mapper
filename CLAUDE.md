...
| `M9K_MODE_{w}x{d}` | `X15Y10N0.M9K_MODE_18x512` | **SCAFFOLDED, np2fasm emission DISABLED.** Parser/loader/bitgen + `_emit_m9k_mode` helper in place (`fuzz/fasm2rbf.py`, `synth/np2fasm.py`). XOR-parity composition + double-emit cancellation tested in `fuzz/test_m9k_init_directive.py` (+2 tests). Two contamination layers identified — (1) **harness IOB footprint** (legacy mining gold mismatch — block-band(smoke) carries IOBs that block-band(nv_zero) doesn't; cancels under same-baseline diff via `fuzz/m9k_mode_remine_w9.py` probe-D 2026-04-16, 51 cells), and (2) **template-style residual** (29 cells intrinsic to altsyncram-direct vs Quartus inferred-RAM code paths — same OPERATION_MODE/WIDTH/NUMWORDS/OUTDATA_REG/READ_DURING_WRITE config, but Quartus emits 29 different MODE cells in upper block band 1718-1738; not IOB, not INIT, not standard altsyncram-parameter controllable). `convert()` keeps MODE emission gated until either (a) a Yosys-emit-matching mining template lands or (b) the residual is parameterized as a sub-flag. Memory: `m9k_mode_template_residual.md`. |
