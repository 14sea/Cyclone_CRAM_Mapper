...
**Not yet implemented**: GCLK clock-pin routing — partial (3 GCLK_BUS spine cells + 6 CLK_pin source-MUX cells found 2026-04-14 via forced-GCLK probes, see `fuzz/clk_force_gclk_probe.py` + `fuzz/clk_pin_gclk_idx_probe.py`; FASM directive split not yet wired). Currently still depends on `nv_zero_global.rbf` baseline. Carry chain (Phase 5.4) and 2D IOB K×LED sweep also pending. The legacy 17-cell `GCLK` directive is **local-clock distribution**, not GCLK_BUS — Quartus' Auto Global Clock heuristic refuses to promote tiny test designs.
