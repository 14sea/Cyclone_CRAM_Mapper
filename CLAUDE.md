...
|-------|-------------|-------|
| `LutCodec` | `write_tt(base_rbf, mask)` → RBF, `read_tt(rbf, zero)`, `from_cram_model(x,y,n)` | **XOR-delta, not absolute!** `mask = target ^ base_tt`. `from_cram_model` uses σ⁻¹ permutation lookup keyed by `(foff, fb8, group)` — **1904 entries** (3-key table, 96% verified, 0% identity fallback), 5-level fallback: exact 3-key → nearest-foff 3-key → exact 2-key → nearest 2-key → identity. Data: `results/sigma_inv_fb8_groups.json`. Known gap: Y=3 (80 positions, wrapped addresses). Entry (94,1,0) fixed 2026-04-21: [1,3,2,0]→[1,2,3,0] (proved by Quartus AND gate gold at X16Y4N0; affects X6/16/26/28 Y4 N0). |
...
| `IOB_ROUTE` | `IOB_ROUTE PIN_E16 -> X16Y4N0.dataa` | OK — pin→LE-port sig lookup. Three buckets in `results/iob_to_slice_sigcache.json` (priority: `padnv_cells` > `single_le_cells` > `absolute_cells`). **`padnv_cells`** (dedup-free, IOB_PAD_NV path): E16→X16Y4N0.dataa (105 cells) + M16→X16Y4N0.datab (13 cells) — derived from Quartus AND gate gold; compose via XOR parity WITHOUT `_iob_route_dedup` stripping. `absolute_cells` reproduces HW-verified pair RBF byte-perfect; `single_le_cells` (15/15 derived but **stale** — all 109 entries fail verification due to directive data drift since derivation). Sweep tool: `scripts/iob_slice_mining/sweep_single_le.py`. |
...
**Status — HW-VALIDATED (2026-04-21)**: Full pipeline produces a registered 2-input AND gate (KEY2 & KEY3 → DFF → LED0) with **0 fabric diffs** vs Quartus gold and **identical silicon behavior** on AX301. This is the first multi-input registered design proven end-to-end through the open toolchain. RBF: `tmp/e2e_test/and_gate_reg_open.rbf`. 86 header-band diff bits (frames 0-16) cause LED1 state difference on unused pin F16 — does not affect design logic. np2fasm emits 10 FASM lines including correct multi-port routing (`IOB_ROUTE PIN_M16 -> X16Y4N0.datab`). Arithmetic designs also work via `LUT_ARITH` (HW-verified 2026-04-13).
