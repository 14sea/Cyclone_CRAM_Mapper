...
16. **γ X=4 cross-LAB — 1-input bypass + 2-input canon BOTH CLOSED 2026-05-13 at X4Y4N0.**
    ...
    - **σ⁻¹ 2-input cross-LAB (P5b/P5c) — STILL BLOCKED at silicon-safety layer** (commit `cf0bef7` 2026-05-15). P5b mined `canon_unique[L]` sidecar via `--canon-unique` flag + `CANON_2INPUT_UNIQUE` table + `canon_2input_unique_aware=1` pragma; P5c shrank per-label cells 175→49–65 by switching to wire4 baseline (`canon_const0_X4Y4N0_wire4.rbf` from `probe_canonicalization_cells.py --constants --wire4`). Apply-time overlap filter in `bitgen` skips canon_unique cells already touched by codec-elsewhere directives (drops cross-LAB analytical mismatch 41→12). But `validate_safe_for_hardware` rejected the cross-LAB build: LAB (4,4) hit 10 LI MUX cells > 9 alternating cap because canon_unique injected 4 wire4-derived LI MUX cells (Quartus uses LI MUX for 4-pin KEY → dataa/b/c/d wire4 routing) that don't fit the cross-LAB no-LI-MUX context. Parallel blocker: `IOB_E16/M16 -> X4Y4N0.dataa/datab` sig-cache missing. **No flash burned.** Memo: `p5c_canon_unique_safety_blocked_2026_05_15`.
