...
|-------|-------------|-------|
| `LutCodec` | `write_tt(base_rbf, mask, *, canon_from='a', canon_to=None, neg_from=False, neg_to=None, bypass=False)` → RBF, `read_tt(rbf, zero, *, bypass_aware=False)`, `from_cram_model(x,y,n)` | **XOR-delta, not absolute!** `mask = target ^ base_tt`. `from_cram_model` uses σ⁻¹ permutation lookup keyed by `(foff, fb8, group)` — 2112 entries, 5-level fallback: exact 3-key → nearest-foff 3-key → exact 2-key → nearest 2-key → identity. Data: `results/sigma_inv_fb8_groups.json`. Residual: fb8=7 × group=4 silicon-geometry blocked (X=8 has no LAB at Y≥12). σ⁻¹'s 16 claimed TT cells per LE are real TT (mining-verified 2026-05-05, `results/real_tt_classification.json`) for **symmetric masks at LE_0**. **X=4 LE_0 σ⁻¹ permutation BROKEN for asymmetric masks** (0x8888→reads 0xC0C0, 0xAAAA→reads 0xF0F0) — see Pitfall #16. Phase 2 verdict 2026-05-13: canonicalization cells are **globally position-invariant** within N=0 LAB columns — codec extension is a single global table (3 sets: 25 axis_a_b + 16 axis_a_c + 7 neg, 35 cells total in shared config, 0 lab_cram). **Silicon-validated 2026-05-13** (two orthogonal flashes): (1) codec-emitted canon_a + neg-toggle (byte-identical to Quartus canon_na md5 `cd847b9c..`) via `canon_apply_transition` direct path — LED matched mask=0x5555 (KEY2-inverted passthrough). (2) **bitgen + `bypass_aware` pragma → canon_b** (byte-identical to Quartus canon_b md5 `85329cf7..`) via `fasm2rbf.bitgen(..., bypass_aware=True)` end-to-end open-toolchain path — LED matched mask=0xCCCC (KEY3 passthrough). Memos: `phase3_4_canon_silicon_validated_2026_05_13.md`, `p1_bypass_aware_silicon_validated_2026_05_13.md`. P1 of plan `imperative-crafting-pumpkin` closed. **Phase 4 (2026-05-13, `7235cac`)**: opt-in `bypass=True` kwarg on `write_tt` skips the per-minterm SRAM emit — for masks with ≤1-input dependence (the 8 `BYPASS_1INPUT_MASKS` plus constants `0x0000`/`0xFFFF`), Quartus routes the input directly to combout (LUT bypass) and emits 0 lab_cram TT cells. Helpers `lut_input_dependence(mask)` and `is_bypass_mask(mask)` enumerate the dependence set. `read_tt(rbf, zero, bypass_aware=True)` recovers the bypass mask from the canon-cell delta when SRAM is silent (known false positive: pristine zero RBF decodes as 0xAAAA). Legacy default behavior unchanged across all kwargs. Memory: `sigma_inv_3key_discovery.md`, `phase3_fix_silicon_validated_2026_05_05.md`, `sigma_inv_extension_unneeded_2026_05_07.md`, `sigma_inv_x4y4n0_not_a_4perm_2026_05_12.md`, `canon_cells_phase2_global_invariant_2026_05_13.md`, `sigma_inv_codec_extension_scoping_2026_05_13.md`, `phase4_bypass_tt_model_2026_05_13.md`. |
...
16. **γ X=4 cross-LAB cluster — Bug #1 strip + Bug #2/#3 codec fixes landed; σ⁻¹ permutation re-mining is the long pole.** Three compounding bugs surfaced 2026-05-10..11:
    ...
    - **σ⁻¹ codec gap (codec-side 1-input closed 2026-05-13 Phase 4; np2fasm consumer wiring + 2-input still open)** — Probe at X4Y4N0 found NO 4-permutation σ⁻¹ fits silicon (24/24 perms ruled out via {0x8888, 0xAAAA, 0xF0F0} mask combo). Direct 0xAAAA-vs-0xF0F0 diff = 16 cells, 0 in TT region: silicon encodes input-permutation choice in ~16 cells **outside** the codec's per-minterm TT model (header band offsets 41-74 + block-band 361605-365142). 0x8888 also uses 12 TT cells vs codec's 8. Implication: σ⁻¹ "re-mining" is a codec-model-extension problem (extract the canonicalization cells per input choice), not a per-position 4-perm table patch. **Phase 2 probe (2026-05-13, commit `4eadf8c`) verdict: GLOBAL POSITION-INVARIANCE for 1-input passthrough canon-cells** — 12 positions covering X∈{4,10,16,22,28}×Y∈{2,17}+extras×N=0 are byte-identical across every diff label (5 axis_*, 4 neg_*). 35-cell layer lives entirely in shared config (header+block_band, 0 lab_cram); negation is axis-independent (single 7-cell flip). Codec extension can be a **single global table**, not per-LE sigcache. Data: `results/canon_cells_phase2_summary.json`. Tools: `scripts/sigma_inv_real_tt_mining/probe_canonicalization_cells.py` (probe; `--2input` mode added Phase 4), `scripts/sigma_inv_real_tt_mining/analyze_canon_phase2.py` (aggregator + verdict). **Phase 3 codec wiring LANDED 2026-05-13** (commits `583baa6`+`c819a61`): `fuzz/bitstream.py` exposes `canon_axis_diff` / `canon_apply_transition` / `canon_classify_transition` + `LutCodec.write_tt(... canon_from, canon_to, neg_from, neg_to)` (legacy-default byte-identical); regression `fuzz/test_canon_cells_codec.py` 8/8. **Phase 4 codec wiring LANDED 2026-05-13** (loop4): `fuzz/bitstream.py` exposes `lut_input_dependence` + `is_bypass_mask` + `BYPASS_1INPUT_MASKS`; `LutCodec.write_tt(... bypass=True)` skips SRAM TT-cell emission (legacy default unchanged). Ground-truth probe confirmed across all 8 Quartus 1-input mask builds at X4Y4N0, the 16 codec minterm cells stay 0 — Quartus LUT-bypass for 1-input passthrough/negation. 2-input AND/OR/XOR P-equivalent class probe shipped at `results/canon_cells_X4Y4N0_2input.json` (NOT codec-wired). Test count 8/8 → 13/13. Memos `phase3_canon_cell_codec_landed_2026_05_13`, `phase4_bypass_tt_model_2026_05_13`. **Scope still open**: end-to-end byte-identity to Quartus canon_*.rbf via np2fasm (consumer-side `bypass=True` wiring); 2-input canon model (codec extension); N>0 invariance (Phase 5). **Pivot for build_test**: use symmetric masks (0x6996 XOR4) at X=4 src-LE, or bypass via single_le sweep for Bug #2. **Do not flash X=4 cross-LAB / src-LE builds with asymmetric masks until np2fasm wires Phase 4 bypass consumer-side.** Y=7 flash 2026-05-11 PM triggered config-controller self-protection reset (flash budget 1/3 used); freshly-mined OUTROUTE_G15 X4Y7N0 (silicon-hostile suspect) **quarantined** 2026-05-13 to `results/quarantine/outroute_g15_X4Y7N0_silicon_reset_2026_05_11.json` — active sigcache now has 38 OUTROUTE_G15 routes; codec hard-fails on missing entry (no silent substitution). Scoping memo `sigma_inv_codec_extension_scoping_2026_05_13`; Phase 2 memo `canon_cells_phase2_global_invariant_2026_05_13`.
    ...
    **1-input bypass class CLOSED 2026-05-13** (commits `5905643` + `b28554b`): np2fasm `--bypass-aware` + fasm2rbf `bypass_aware=True` kwarg + post-loop `canon_apply_transition` produce RBFs **byte-identical to Quartus `canon_*_X4Y4N0.rbf` for all 8 BYPASS_1INPUT_MASKS + 2 constants**. Silicon-validated via canon_b bitgen flash (KEY3 idle ON / press OFF; mask=0xCCCC). Regression `fuzz/test_bypass_byte_identity.py` 5/5; pre-commit hook gates it. **2-input asymmetric masks (Pitfall #16 main axis) remain pending P2** — codec extension needs `results/canon_cells_X4Y4N0_2input{,_neg}.json` datasets + multi-position invariance verdict. Memos: `p1_bypass_aware_silicon_validated_2026_05_13`, `phase3_4_canon_silicon_validated_2026_05_13`.
