...
16. **Cross-LAB R4 sigcache × IOB_PAD_NV / LAB_CLK_SEL_LE XOR-cancel** — Bug #1 PARTIALLY FIXED 2026-05-11 (`df335d8`). Plan D' factory mining bakes IOB+CLK infra cells into the cross-LAB ROUTE diff; runtime `IOB_PAD_NV` + `LAB_CLK_SEL_LE` emit the same cells → double XOR-flip → cells silently absent → silicon failure. Affects 96/109 X=4 entries (>100 cells / ≥30-cell overlap); same class expected at X=10/16/22/25/28/31. Fix: `scripts/sigcache_remine/strip_iob_overlap.py --apply --column X` subtracts `IOB_PAD_NV ∪ LAB_CLK_SEL_LE(s/d) ∪ LAB_CLK_SEL(s/d)` per-entry; idempotent, patches `nv_route_cells.json` + `route_cells_full.json` in lockstep. **Re-run after every `fuzz/nv_sig_cache_merge.py`.** Post-fix `build_test` (entry `4,4,0→4,21,0,dataa`): 63 missing → 8 fabric + 13 header residuals.  **Codec follow-up landed 2026-05-11 (`03b91c6`)** closes 4 more fabric cells (ROUTE×OUTROUTE_G15 strip-at-emit + Phase 3 LI MUX skip extended to IOB_PAD_NV / OUTROUTE_G15, with X33Y4 shim exemption) → 8 fabric → 4 fabric residuals.  ζ 7/7, test_phase3 11/11, green_zone 731/731, roundtrip 2035/2035; silicon-validated md5s preserved (W=23 `905dfc85`, probe2 `d4073d2e`, cross_lab_x33 `47ad9ed9`). Remaining silicon blocker: σ⁻¹ port permutation at X4 LE_0 (4 cells; deep — only Y=7/Y=19 round-trip cleanly with asymmetric masks; Pitfall #13 closure only tested symmetric XOR4 mask). **Do not flash X=4 cross-LAB builds until σ⁻¹ permutation closed.** Test fixture: `scripts/sigcache_validation/build_test.py`. Memos: `gamma_silicon_validation_codec_bug_2026_05_10`, `gamma_bug1_strip_fix_landed_2026_05_11`. Pitfall #14 reframed: context dependency applies in every design where both directives emit.
