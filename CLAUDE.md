...
| `LAB_CLK_SEL_LE` | `LAB_CLK_SEL_LE X10Y4N0` | OK — per-LE CLK_SEL layer (XOR, disjoint from `LAB_CLK_SEL`). **Functionally required** (HW verified 2026-04-14 at LAB(10,4).N=0: N-invariant alone = LED-always-off). Data: `results/clk_lab_sel_per_le.json` derived by `fuzz/clk_lab_sel_per_le.py`. N ∈ {0, 2, 4} mined for **all 14 LABs** as of 2026-04-15 (12 extended via `fuzz/clk_lab_sel_n2_batch.py`, 4-way parallel). To re-mine after probe edits or to extend N further, run `fuzz/clk_lab_sel_probe.py --lab X,Y --work tmp/<unique>` then `fuzz/clk_lab_sel_per_le.py`. |
...
| `M9K_MODE_{w}x{d}` | `X15Y10N0.M9K_MODE_18x512` | **SCAFFOLDED, np2fasm emission DISABLED.** Parser/loader/bitgen + `_emit_m9k_mode` helper in place (`fuzz/fasm2rbf.py`, `synth/np2fasm.py`). XOR-parity composition + double-emit cancellation tested in `fuzz/test_m9k_init_directive.py` (+2 tests). Per-site cell data in `results/m9k_mode_bits.json` (mined by `fuzz/m9k_mode_mine.py` from per-site `m9k_calib18b_*` RBFs vs `nv_zero_global`) is **contaminated** by harness routing/IOB/local-clock infra — emitting on the smoke build worsened m9k-band diff (43→86 cells, 66 false flips, 35 missed). `convert()` skips MODE emission until single-M9K Quartus builds with a shared IOB+clock harness give clean per-site diffs. |
...
**M9K_MODE scaffolded + DSPMULT N-invariance probe (2026-04-16)**: M9K per-site enable directive `M9K_MODE_{w}x{d}` is wired through codec (parser/loader/bitgen in `fuzz/fasm2rbf.py`, helper `_emit_m9k_mode` in `synth/np2fasm.py`, 11 + 7 unit tests). `convert()` emission is **gated off** — per-site `m9k_calib18b_*` mining RBFs each carried their own LED+clock IOB harness, so the per-site→`nv_zero_global` diff is contaminated with harness routing/IOB infra. Emitting the mined cells worsened the smoke build's m9k-band byte diff (43 → 86; 66 false flips, 35 missed). Re-enable requires single-M9K Quartus builds with a *shared* IOB+clock harness. The same harness-leak class hit the 42-site DSPMULT block-band probe (`fuzz/dspmult_persite_analyze.py` → `results/dspmult_persite_analyze.json`): median 223-cell data-band leak per RBF; the 29 universal cells (intersection across all 42 sites) are reusable as a "DSPMULT_GLOBAL_ON" template, but per-site cells (6-14 each) are noise. The probe also confirmed N-invariance: 17/21 Y rows have byte-identical N=0/N=1 block-band sets (avg sym-diff 1.5 cells/pair). Cross-cutting rule: per-site mining MUST use a shared harness; gate any new per-site directive on a data-band leak probe before enabling np2fasm emission.
