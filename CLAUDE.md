...
**Not yet implemented**: GCLK clock-pin routing — partially mapped. GCLK source encoding is **per-pin one-hot** (no universal spine): each dedicated CLK pin has its own cell set that must be flipped to activate it as a global clock source. Triangulation across E1/R8/N1 (2026-04-14) gave 0-cell all-pair intersection; cross-pin forced-vs-auto overlap is 0 cells. Mapped so far: `PIN_E1` → 3 cells at frames 34-35, `PIN_R8` → 5 cells at frames 58/61/834/837/846. Per-LAB CLK_SEL (Gap B) also partially mapped via `fuzz/clk_lab_sel_probe.py`: LAB(10,4)=26 cells, LAB(10,16)=53 cells, LAB(22,10)=45 cells — not yet a clean formula (N-invariant deltas still contain shared row-GCLK-tree cells), but directly usable as FASM union payloads since flipping shared cells is idempotent. Probes: `fuzz/clk_force_gclk_probe.py`, `fuzz/clk_pin_gclk_idx_probe.py`, `fuzz/clk_pin_triangulate_probe.py`, `fuzz/clk_iob_subtract_probe.py`, `fuzz/clk_cross_pin_spine_check.py`, `fuzz/clk_lab_sel_probe.py`. FASM directive split (per-pin activate + per-LAB CLK_SEL) not yet wired — currently the bitgen still depends on `nv_zero_global.rbf` baseline. Carry chain (Phase 5.4) and 2D IOB K×LED sweep also pending. The legacy 17-cell `GCLK` directive is **local-clock distribution**, not GCLK source — Quartus' Auto Global Clock heuristic refuses to promote tiny test designs.
