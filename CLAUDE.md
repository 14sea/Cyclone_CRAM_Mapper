...
| `M9K_MODE_{w}x{d}[_template]` | `X15Y10N0.M9K_MODE_9x512_quartus_gold` | ⚠️ **All codec-emission paths silicon-broken** as of 2026-04-30. (1) `_quartus_gold*` + `--base nv` (production np2fasm): bucket mined relative to `matched_baseline` but applied as XOR onto `nv_zero_global` → ~85% bit inversion. Every codec-emission flash silicon-resets. See memory `m9k_mode_codec_silicon_broken_2026_04_25` + `m9k_mode_d2_falsified_2026_04_26`. (2) `_inferred_goldintersect` + `--base pure`: previously claimed HW-validated based on overlay tests at w=9/w=18 (2026-04-17, 2026-04-24), but those proved fabric-safety only — NOT mode-correctness. ζ-swap silicon test 2026-04-30 (memory `m9k_mode_v5_finding_gi_codec_unnecessary_2026_04_30`) proved the gi codec targets cells the M9K mode hardware doesn't read; v3 (gi alone, no extras, applied onto m9k_blink) silicon-FAILED with M9K stuck reading constant. v5 (drop gi entirely, keep only the 12 non-inferred Quartus block-band cells per site) silicon-PASSED — gi cells are noise, the real M9K mode encoding lives in cells the inferred mining systematically drops (cells where matched_baseline == nv_zero_global at bit=1). (3) **Recommended replacement**: per-site `m9k_blink_diff_nv` bucket — mine `(m9k_blink_at_site ⊕ nv_zero_global)` in block-band data → ~12 cells per site → emit directly via new directive. Mining cost: 1 Quartus build per (mode, w, d, site). Tooling started: `scripts/m9k_blink_full_build.py`. Quartus-gold flashes (NOT codec-emission) still work for direct-flash use cases. `quartus_gold` buckets mined via `scripts/m9k_mode_quartus_gold_mine.py` (single) or `scripts/m9k_mode_quartus_gold_batch.py --neorv32` (sweep). **np2fasm gates** (`_M9K_MODE_FUNCTIONAL_VALIDATED_{SP,SDP,TDP}` × lazy-loaded site set from `results/m9k_mode_bits.json`) covers all 27 NEORV32 M9K sites + X15 Y10..Y14 SP (8,64) cache — but emission is **silicon-broken** under both gold and gi paths until `_diff_nv` lands. SDP/TDP modes via `--mode sdp|tdp`; `np2fasm._emit_m9k_mode` dispatches on techmap MODE param. Full history: memory `m9k_mode_*` entries. |
