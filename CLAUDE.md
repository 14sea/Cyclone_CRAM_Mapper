...
|-------|-------------|-------|
| `LutCodec` | `write_tt(base_rbf, mask, *, canon_from='a', canon_to=None, neg_from=False, neg_to=None, bypass=False, canon_2input=None)` → RBF, `read_tt(rbf, zero, *, bypass_aware=False)`, `from_cram_model(x,y,n)`, `predict_sram(mask)` | **XOR-delta, not absolute** (`mask = target ^ base_tt`). σ⁻¹ permutation lookup via `(foff, fb8, group)` 3-key, 5-level fallback (`results/sigma_inv_fb8_groups.json`, 2112 entries). σ⁻¹ valid for symmetric masks at LE_0; **2-input asymmetric closed at X4Y4N0 via P2 absolute table** — see Pitfall #16. **P1 (1-input)**: Phase 3 canon-cell layer landed 2026-05-13 (35-cell global table, shared config, 0 lab_cram) + Phase 4 `bypass=True` skips SRAM emit for 8 `BYPASS_1INPUT_MASKS` ∪ {0x0000, 0xFFFF}. **P2 (2-input)**: `canon_2input='a&b'..` kwarg (mutex w/ `canon_to`) XOR-applies absolute cells from `CANON_2INPUT_ABSOLUTE[(x,y,n)][label]` (~240 cells/label; 24 labels: 18 perm AND/OR/XOR + 6 neg). Position scope: **X4Y4N0 only**; raise KeyError elsewhere. Helpers: `lut_input_dependence`, `is_bypass_mask`, `canon_axis_diff`, `canon_apply_transition`, `canon_classify_transition`, `canon_2input_label_for_mask`, `canon_2input_apply`, `canon_2input_cells`, `canon_2input_unique_cells`, `canon_2input_unique_apply` (P5c wire4-baseline-subtracted parallel sidecar `CANON_2INPUT_UNIQUE` from `canon_2input_codec_table_unique.json`; opt-in via `canon_2input_unique_aware=1` pragma + P5c apply-time codec-overlap filter + P5d per-LAB LI envelope filter via `li_lab_for_offset` — **P5d SILICON-FAILED for cross-LAB 2026-05-21**, do not enable `canon_2input_unique_aware` for cross-LAB builds; see Pitfall #16 P5d row). Silicon-validated: canon_na via direct apply (mask=0x5555); canon_b via `bitgen(bypass_aware=True)` (mask=0xCCCC); **canon_naandb via `bitgen(canon_2input_aware=True)` (mask=0x4444 = !KEY1&KEY2, 2026-05-13)**. Legacy defaults byte-identical. Memos: `phase3_canon_cell_codec_landed_2026_05_13`, `phase4_bypass_tt_model_2026_05_13`, `p1_bypass_aware_silicon_validated_2026_05_13`, `p2_canon_2input_silicon_validated_2026_05_13`, `p5b_canon_unique_landed_2026_05_15`, `p5c_canon_unique_safety_blocked_2026_05_15`. |
| `RouteCodec` | `read_c4/r4/r24/local_interconnect()`, `apply_routing(zero_data, switches)` | Round-trip verified. LI writes take explicit `pairs`, NOT i_idx |
...
**SAFETY**: `validate_safe_for_hardware(rbf, zero)` checks LI MUX envelopes (P0 anchor / P8 tail / mode = paired/alternating/driver*/edge_*).  Recognized modes include `driver_single` (`{8: [single_base]}`) silicon-validated against Quartus gold (memory `step_3_substantially_closed_2026_05_02`).  Always call before flashing — but validator scope is LI MUX class only; it does NOT check header band consistency, non-LAB-column writes, GCLK distribution completeness, or output-buffer drive contention (memory `path_alpha_led_blink_silicon_failed_2026_05_02`).  For silicon-functional validation, also cross-check vs Quartus gold cell set or use ζ pipeline.  **LI-MUX composition gate** (`fuzz/li_mux_conflict_gate.py`, gold-anchored): when a build combines a LUT/canon directive with routing directives (ROUTE/IOB_ROUTE/SRC) at the same LE, those LI-MUX cells are co-determined by XOR composition and `validate_safe_for_hardware` is BLIND to the result.  `li_mux_gate(fasm, base, composed_rbf=, gold_rbf=)` FAILs on gold byte-identity mismatch at the co-determined cells and refuses to certify when no gold is supplied — this is the under-driven-LE_A class that silicon-killed P5d (Track D 2026-05-28).  No gold-free local authority exists (the correct LI value is global-routing-dependent — Quartus's own golds disagree at the same (off,bp) across designs).  Memory: `track_d_xlab_codec_quartus_forensic_2026_05_28`.
...
| `IOB_ROUTE` | `IOB_ROUTE PIN_E16 -> X16Y4N0.dataa` | Two apply-paths: default live (padnv_cells > absolute_cells; absolute_cells get dedup + `off<5282` hdr-skip, padnv_cells skip both); legacy (`bitgen(legacy_iob_route=True)` for simple_led single-LE). np2fasm pragma `# fasm2rbf: legacy_iob_route=1`. **Apply order**: post-all_luts before patch_crc (Track B1 reorder 2026-05-21) — XOR composition with OUTROUTE_G15 + LUT Phase 1 now correctly produces gold byte at LI-aliased cells.  Mine X4Y4N0: `mine_padnv_x4y4n0.py` (P1/P2 pragma-driven, σ⁻¹ contamination bypassed; E16→dataa + M16→datab landed 2026-05-21). Memory: `iob_route_hdr_skip_pivot_2026_05_11`, `gamma_iob_x4y4_padnv_mining_2026_05_21`. |
...
16. **γ X=4 cross-LAB — 1-input bypass + 2-input canon BOTH CLOSED 2026-05-13 at X4Y4N0.**
    ...
    - **σ⁻¹ canon-cell layer 1-input** — CLOSED Phase 3+4 (`583baa6`/`c819a61`/`5905643`/`b28554b`): codec `canon_axis_diff` / `canon_apply_transition` / `canon_classify_transition` + `write_tt(canon_from, canon_to, neg_from, neg_to, bypass)` + `lut_input_dependence` / `is_bypass_mask` / `BYPASS_1INPUT_MASKS`. np2fasm `--bypass-aware` + fasm2rbf `bypass_aware=True` pragma → byte-identical to Quartus `canon_*_X4Y4N0.rbf` for all 8 1-input + 2 constants. Silicon-validated canon_b (mask=0xCCCC). Phase 2 verdict: GLOBAL POSITION-INVARIANT (12 positions, shared config).
    - **σ⁻¹ 2-input asymmetric (P2) — CLOSED at X4Y4N0** (commit `82ecd87` 2026-05-13): codec `canon_2input_label_for_mask` / `canon_2input_cells` / `canon_2input_apply` + `LutCodec.write_tt(canon_2input=label)` + `CANON_2INPUT_ABSOLUTE[(x,y,n)][label]` per-position table (~240 cells/label; 18 perm AND/OR/XOR + 6 neg = 24 labels). Sidesteps σ⁻¹ predict_sram gap (~108 lab_cram cells) by mining the full delta. fasm2rbf `canon_2input_aware=True` pragma byte-identical for all 24 labels. **Silicon-validated mask=0x4444 = !KEY1 & KEY2** (md5 `bd373139..`) on AX301 2026-05-13. **Position scope: X4Y4N0 ONLY** — KeyError at unmined positions (5-position pair-diff probe shows 2-input is NOT globally position-invariant; per-position mining required). Mining: `scripts/sigma_inv_real_tt_mining/build_canon_2input_codec_table.py`. Regression `fuzz/test_canon_2input_codec.py` 14/14 + pre-commit gate.
    - **σ⁻¹ 2-input per-position canon table (Path 5 — d1pp) — LANDED 2026-05-21; CRTM Part-3 WRAPPED 2026-05-28.** `results/canon_2input_codec_table_d1pp.json` (md5 `4dc37fbab8828034acda1270b19fa5e5`) encodes per-position canon-cell XOR delta for 14 pin-friendly D1++ positions = **44 (x,y,n,mask) entries** (per-mask `0x6996×14, 0xDEAD×14, 0x4444×14, 0xFFFF×2`; 58 golds in `results/golden_rbf_modeH/`).  Consumer formula (mode H lutcodec.c): `Quartus_mask_M_at_(x,y,n) = baseline_mask0 XOR predict_sram(M) XOR canon_cells[(x,y,n)][M]`, then `patch_rbf_crc`.  Verifier `scripts/mode_h/verify_canon_d1pp_table.py` reconstructs **44/44 byte-identical** to Quartus Lite 21.1 gold (excluding D4 per-build variable bytes `[0x29..0x35)` + `[0x49..0x4B)`; CRC recomputed via `patch_rbf_crc`).  **Scope caveat: lossless mine→verify ROUND-TRIP gate (same builds mined+checked), NOT hold-out/silicon validation** (negative control confirms the gate is live; tautology by construction — do not cite as model-generalization or silicon proof).  Mining tool: `scripts/mode_h/build_canon_d1pp_table.py` (no Quartus compiles; mines from existing D1++ `.cells` diffs).  Scope: N=0 only, 14 positions X∈{10,16,22,28} × Y∈{2,8,14,21} with one substitution (X28Y17N0 ← X28Y17N14).  Part-3 commits `8bd29e1` (0xDEAD all-14) → `d2d0762` (hook de-hardcode) → `a54d289` (0x4444 all-14); more masks/positions = same recipe + Quartus compiles.  Replaces P5d's per-LAB filter at non-X4Y4 positions; X4Y4N0 still uses `CANON_2INPUT_ABSOLUTE` (P2 silicon-validated path). Memo: `path5_canon_d1pp_table_landed_2026_05_21`.
    - **σ⁻¹ 2-input cross-LAB (P5b/P5c/P5d) — SILICON-FAILED 2026-05-21 PM.** P5d pre-flash gates green (safety 0 violations, analytical 10/61 CANDIDATE) but flash of `ae9aaef6cba4d48ab4a10f78ac994aef` produced LED0 stuck ON at idle, keys unresponsive, no UART. Root cause: the per-LAB LI filter conflated codec's *outbound* cross-LAB ROUTE LI cells at LAB(4,4) (5 cells from sigcache `4,4,0->4,21,0,dataa`) with canon_unique's *inbound* IOB→LE_A input-routing LI cells at the same LAB (4 cells at `0x9354/0x9427/0x94F8/0x95CB` bp=6 — empirically confirmed absent in flashed RBF). Same LAB tag, disjoint at (off, bp), but semantically different routes — filter dropped the inbound, undriving LE_A on silicon. **Do NOT enable `canon_2input_unique_aware` for cross-LAB builds.** Single-LE wire4 byte-identity path is unaffected (codec emits 0 LI cells at LAB(4,4) in single-LE context → filter doesn't fire; 24/24 + 5/5 regression preserved). **Track B1 (2026-05-21 PM) unblocks the parallel `IOB_E16/M16 → X4Y4N0.dataa/datab` sig-cache** (`mine_padnv_x4y4n0.py` now P1/P2 pragma-driven, 236+280 cells injected into padnv_cells; bitgen iob_routes reordered to post-all_luts) — cross-LAB builds at X4Y4 can now use the P2 silicon-validated canon_2input absolute path WITHOUT needing the P5d filter at all.  Path 5 (`canon_2input_codec_table_d1pp.json`) covers non-X4Y4 pin-friendly positions. **Flash budget 0/3 remaining.** History memos: `p5d_per_lab_li_filter_2026_05_21` (pre-flash; SUPERSEDED), `p5d_silicon_failed_2026_05_21` (failure analysis + forensic), `path5_canon_d1pp_table_landed_2026_05_21` (canon-cell replacement), `gamma_iob_x4y4_padnv_mining_2026_05_21` (IOB sig-cache replacement).
    - **OUTROUTE_G15 X4Y7N0 quarantined** 2026-05-13 to `results/quarantine/` after silicon-reset; codec hard-fails on missing.
    - **Pivot guidance (updated 2026-05-15)**: at X4Y4N0, asymmetric masks (e.g. 0x4444, 0xBBBB, 0xA0A0) are codec-supported via `canon_2input_aware=True` for **single-LE wire4 designs only** (silicon-validated). Cross-LAB designs at X=4 still BLOCKED — analytical gate green but safety validator catches LI MUX class. Other X=4 src-LE positions (Y∈{2,7,17,21}) still require symmetric 0x6996 workaround OR per-position canon-2input mining. **N>0 (P4 axis) SILICON-VALIDATED 2026-05-31** at X10Y2N2 mask 0x4444 (gold md5 `f4f971d4`, idle LED OFF / KEY1-alone → ON); codec formula `baseline⊕predict_sram(M)⊕canon` reconstructs the N>0 gold byte-identical — but **canon is per-N (N-invariance FALSIFIED)**, so other N need per-N mining, exactly like the per-position nature here. Memo: `p4_n_invariance_2026_05_29`. **Pre-flash protocol:** always run `RouteCodec().validate_safe_for_hardware(rbf, nv_zero, raise_on_fail=False)` and check empty before declaring an analytical gate "candidate" — analytical mismatch ≤ threshold is necessary but not sufficient.
    - **Latent CLI bug fixed (`cf0bef7` 2026-05-15)**: `fasm2rbf.main()` was silently dropping all FASM pragmas (no `parse_pragmas` call). Pre-fix, CLI builds with bypass_aware/canon_*_aware pragmas were no-ops. Fixed; only direct `bitgen()` callers (regression tests, ζ pipeline, build_open scripts) had ever honored pragmas. ζ regression unaffected (pragma-less FASM).
    Test fixture: `scripts/sigcache_validation/build_test.py` (now with `--le-a-mask` + `--canon-2input` CLI flags, no uncommitted Verilog edit needed). Memos: `gamma_*`, `iob_route_hdr_skip_pivot_2026_05_11`, `sigma_inv_x4y4n0_not_a_4perm_2026_05_12`, `phase3_canon_cell_codec_landed_2026_05_13`, `phase4_bypass_tt_model_2026_05_13`, `p1_bypass_aware_silicon_validated_2026_05_13`, `p2_canon_2input_silicon_validated_2026_05_13`, `phase4_p1_p5_landed_2026_05_13`, `p5b_canon_unique_landed_2026_05_15`, `p5c_canon_unique_safety_blocked_2026_05_15`.
