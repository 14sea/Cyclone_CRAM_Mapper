...
| `IOB_CLK_INPUT` | `IOB_CLK_INPUT PIN_E1` | OK for 12 pins on F17 — hdr delta activating a clock-bank pin as GCLK driver. Extensible via `scripts/iob_slice_mining/compute_clk_pin_hdr.py --build --pin {PIN}` → `results/iob_clk_pin_hdr_cells.json`. **PIN_E1 entry is legacy 16-cell mining**; 2026-05-03 re-mining attempt produced 40 cells but those were anchor (`iob_in_E16.rbf`) artifacts — reverted. Caveat for callers: data is calibrated against `IOB_BASELINE_NV` (129-cell) baseline; when used together with `IOB_PAD_NV` (139-cell, M16-active assumed), 11 cells overlap and double-flip — corrected by `IOB_RESERVE_PIN_M16` Group B when M16 is reserved. |
