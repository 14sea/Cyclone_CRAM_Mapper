...
| `M9K_MODE_{w}x{d}[_template]` | `X15Y10N0.M9K_MODE_9x512_quartus_gold` | **Content-correct quartus_gold re-mine landed 2026-04-24d; per-width functional HW validation pending.** `quartus_gold` buckets mined via `scripts/m9k_mode_quartus_gold_mine.py` — per (w,d) at X15_Y10_N0, intersection of 3 Quartus altsyncram variants (different INIT + read-pipe) vs a pinout-matched no-M9K baseline, block-band only. Sizes: (4,2048)=19, (9,512)=52, (18,512)=59, (9,1024)=43, (36,256)=68. Algebraic cross-check: for (9,512) the 52-cell bucket covers 46/52 of `tmp/m9k_smoke/ram_9x512.rbf`'s block-band diff vs its matched baseline; for (4,2048) it covers 14/19 of `tmp/m9k_dpath_w4x2048/m9k_blink.rbf`'s diff (both use different Verilog templates; the shared subset is the real mode encoding). Loadable via `M9K_MODE_{w}x{d}_quartus_gold` — codec + round-trip XOR-cancel verified for all 5 widths (`test_m9k_mode_quartus_gold_bucket_all_widths`). np2fasm two-tier gate: `_M9K_MODE_FUNCTIONAL_VALIDATED` (empty, widths join after data-path HW PASS — emit `_quartus_gold`) and legacy `_M9K_MODE_HW_VALIDATED` = {(9,512),(18,512),(9,1024),(36,256)} (fabric-safe only — still emit `_inferred_goldintersect`, annotated as non-functional). (4,2048) stays skipped in np2fasm. Memory: `m9k_mode_quartus_gold_mining_landed.md`, `m9k_mode_gi_bucket_not_quartus_encoding.md`.
