...
| `LUT_ARITH_MULTI_LAB` | `LUT_ARITH_MULTI_LAB WIDTH=17` | ⚠️ **SILICON-BROKEN** — widths 17..32 multi-LAB chain activation.  Codec parses + emits cleanly, but `results/arith_blockband_by_width.json` `multi_lab["16+N"]` data was mined from Quartus designs where Quartus REJECTED the FF_X{LX}_Y{ly}_N{n} location constraints (Warning 15706: `Node Q[i]~reg0 does not exist in design`).  Cells reflect Quartus's free placement (X=9/X=13/X=14 area, varying per width), NOT LAB(4,18)+(4,17) as docs claim.  Hand-FASM at LAB(4,18)+(4,17) → arith mode activates at wrong LABs → LEs run as LUT_TT=0x0000 → output 0 always → LED off (silicon-confirmed 2026-05-03 with W=17 + W=23).  np2fasm consumer NOT wired (Yosys `$alu` techmap caps at single-LAB).  Re-mine fix: investigate via `quartus_cdb` atom traversal, update `gen_widths_17_32.py` QSF emission, re-run sweep, verify each width's fit.log lacks Warning 15706.  See memory `multi_lab_codec_broken_2026_05_03.md` (definitive root cause) + `arith_width_17_32_landed.md` (historical, claim-as-mined). |
