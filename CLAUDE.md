...
| `M9K_MODE_{w}x{d}[_template]` | `X15Y10N0.M9K_MODE_9x512_quartus_gold` | **HW-validated (functional) across all 5 widths on AX301 silicon 2026-04-24d.** `quartus_gold` buckets mined via `scripts/m9k_mode_quartus_gold_mine.py` — per (w,d) at X15_Y10_N0, intersection of 3 Quartus altsyncram variants (different INIT + read-pipe) vs a pinout-matched no-M9K baseline, block-band only. Sizes: (4,2048)=19, (9,512)=52, (18,512)=59, (9,1024)=43, (36,256)=68. Per-width data-path HW sweep: for each (w,d) an `m9k_blink_{w}x{d}.v` counter design (`scripts/m9k_blink_build.py`) was Quartus-built at X15_Y10_N0 and flashed on AX301; every one blinked LED0 stably at the expected ~0.186 Hz cadence (2.7 s on / 2.7 s off), confirming the silicon supports each mode. Loadable via `M9K_MODE_{w}x{d}_quartus_gold` — codec + round-trip XOR-cancel verified (`test_m9k_mode_quartus_gold_bucket_all_widths`). np2fasm two-tier gate: `_M9K_MODE_FUNCTIONAL_VALIDATED = {(4,2048),(9,512),(18,512),(9,1024),(36,256)}` — all 5 widths emit `_quartus_gold`. Legacy `_M9K_MODE_HW_VALIDATED` retained as the fabric-safe gate for any caller still asking for `_inferred_goldintersect`. Memory: `m9k_mode_quartus_gold_mining_landed.md`, `m9k_mode_gi_bucket_not_quartus_encoding.md`.
