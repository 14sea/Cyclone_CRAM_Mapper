...
| `IOB_ROUTE` | `IOB_ROUTE PIN_E16 -> X16Y4N0.dataa` | OK — pin→LE-port sig lookup. Two **live** buckets in `results/iob_to_slice_sigcache.json` (priority: `padnv_cells` > `absolute_cells`). **`padnv_cells`** (dedup-free, IOB_PAD_NV path): E16→X16Y4N0.dataa (105 cells) + M16→X16Y4N0.datab (13 cells) — derived from Quartus AND gate gold; compose via XOR parity WITHOUT `_iob_route_dedup` stripping. `absolute_cells` (15 entries at X∈{10,16} Y∈{4,10}) reproduces HW-verified pair RBF byte-perfect, requires dedup. **Quarantined 2026-04-24**: `single_le_cells_stale` (109 entries derived 2026-04-15) — full-RBF reconstruction now misses by 120 bytes after directive-stack drift; fasm2rbf + np2fasm no longer consult it. 94 of those 109 keys exist ONLY in the stale bucket (X=3 Y=17/18/19 route families); those combos are intentionally unroutable until re-mined. Reinstate path: extend `scripts/iob_slice_mining/sweep_single_le.py` `TARGETS`, re-run, rename key back. |
